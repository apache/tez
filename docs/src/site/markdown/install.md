...
the working directory of the Tez runtime and they will automatically be included
into the classpath. For archives i.e. files whose names end with generally known
...
configure `tez.lib.uris.classpath` to ensure that the nested directory structure
...
ensure that the versions of Tez and Hadoop being used are compatible. To do this
...
- tez-site.xml configuration
  ...
  - In tez.lib.uris, the text immediately following the '#' symbol is the
    ...
    If no fragment is given, the symlink will be set to the name of the archive.
