...
public class TestZkFrameworkClient {
  ...
  public void testGetApplicationReportWithRegisteredAM() throws Exception {
    ...
    int testPort = 12345;
    registerMockAM(tezConf, appId, testHost, testPort);
    ...
    assertEquals("Host should match", testHost, report.getHost());
    ...
    assertEquals("AM host should be cached", testHost, zkFrameworkClient.getAmHost());
  ...
  private void registerMockAM(TezConfiguration tezConf, ApplicationId appId, String host, int port) throws Exception {
    // Create AM record and publish it directly to ZooKeeper
    AMRecord amRecord = new AMRecord(appId, host, port, "test-tracking-url");
