...
import org.apache.tez.mapreduce.hadoop.*;
...
extends org.apache.tez.common.TezTask {
  ...
  private final TaskSplitIndex splitMetaInfo;
  ...
  public MRTask(TezTask context) {
    super(context.getTaskAttemptId(), context.getUser(), context.getJobName(),
        context.getTaskModuleClassName());
    MRTaskContext tc = (MRTaskContext)context;
    splitMetaInfo = 
        new TaskSplitIndex(tc.getSplitLocation(), tc.getSplitOffset());
    ...
    status =
        new MRTaskStatus(
            getTaskAttemptId(),
            counters,
            MRTaskType.fromString(getTaskAttemptId().getTaskType()) == MRTaskType.MAP ? Phase.MAP
                : Phase.SHUFFLE
        );
  ...
  public void initTask(JobConf job, TezJobID tezJobID, 
      MRTaskReporter mrReporter,
      boolean useNewApi) throws IOException, 
                                InterruptedException {
    this.jobConf = job;
    this.jobContext = new JobContextImpl(job, tezJobID, mrReporter);
  ...
  public void localizeConfiguration(JobConf jobConf) 
    ...
    jobConf.set(JobContext.ID, getTaskAttemptId().getJobID().toString());
  }
