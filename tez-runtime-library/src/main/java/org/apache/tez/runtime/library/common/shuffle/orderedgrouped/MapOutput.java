...
class MapOutput {
  ...
  public void commit() throws IOException {
    if (type == Type.MEMORY) {
      ...
      merger.getLocalFileSystem().rename(tmpOutputPath, outputPath.getPath());
  ...
  public void abort() {
    if (type == Type.MEMORY) {
      merger.unreserve(byteStream.getBuffer().length);
    } else if (type == Type.DISK) {
      try {
        merger.getLocalFileSystem().delete(tmpOutputPath, true);
