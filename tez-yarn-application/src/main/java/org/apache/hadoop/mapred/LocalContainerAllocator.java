...
public class LocalContainerAllocator extends AbstractService
  ...
  public LocalContainerAllocator(AppContext appContext, JobId jobId,
      ...
      TezTaskUmbilicalProtocol taskUmbilical,
  ...
  void handleTaLaunchRequest(AMSchedulerTALaunchRequestEvent event) {
    ...
    // Register a ContainerId, so that the TAL can handle pings etc. 
    taskAttemptListener.registerRunningContainer(amContainerId);
    

    if (event.getAttemptID().getTaskId().getTaskType() == TaskType.MAP) {
      JobEventCounterUpdate jce = new JobEventCounterUpdate(event
    ...
    org.apache.hadoop.mapreduce.v2.app2.job.Task ytask = job.getTask(attemptID
        .getTaskId());
    // classic mapred Task:
    MRTaskContext remoteTaskContext = launchEv.getRemoteTaskContext();
    ...
    appContext.getEventHandler().handle(
        new TaskAttemptEventStartedRemotely(attemptID, amContainerId,
    ...
    try {
      if (isMapTask(remoteTaskContext.getTaskAttemptId())
          || isReduceTask(remoteTaskContext.getTaskAttemptId()))
        if (attemptID.getTaskId().getTaskType() == TaskType.MAP
            || attemptID.getTaskId().getTaskType() == TaskType.REDUCE) {
          JobEventCounterUpdate jce = new JobEventCounterUpdate(attemptID
              .getTaskId().getJobId());
      ...
      taskAttemptListener.registerTaskAttempt(event.getAttemptID(), amContainerId);
      runSubtask(remoteTaskContext, ytask.getType(), attemptID, numMapTasks,
          (numReduceTasks > 0));
  ...
  public void handleTaStopRequest(AMSchedulerEventTAEnded sEvent) {
    ...
    taskAttemptListener.unregisterTaskAttempt(sEvent.getAttemptID());
  }
  ...
  private class SubtaskRunner implements Runnable {
    ...
    private void runSubtask(MRTaskContext task,
                            final TaskType taskType,
                            TaskAttemptId attemptID,
                            final int numMapTasks,
                            boolean renameOutputs)
    throws RuntimeException, IOException {
      // TODO TEZAM3 Cleanup all of this. Shuold be possible to abandon classic IDs in the MRAM
      ...
      try {
        ...
        conf.set(JobContext.TASK_ID, task.getTaskAttemptId().toString());
        conf.set(JobContext.TASK_ATTEMPT_ID, classicAttemptID.toString());
        conf.setBoolean(JobContext.TASK_ISMAP, (taskType == TaskType.MAP));
        conf.setInt(JobContext.TASK_PARTITION, getPartition(task.getTaskAttemptId()));
        ...
        if (taskType == TaskType.MAP) {
          ...
          // TODO VERIFY - Move functionality is as it should be.
          AbstractModule module = null;
          if (renameOutputs) {
            module = new InitialTaskWithLocalSort();
          } else {
            module = new InitialTask();
          }
          Injector injector = Guice.createInjector(module);
          TezEngineFactory factory = injector.getInstance(TezEngineFactory.class);
          t = factory.createTask(task);
          t.initialize(conf, umbilical);
          SimpleInput input = ((SimpleInput)t.getInput());
          t.getProcessor().process(input, t.getOutput());
          ...
          // TODO TEZAM3 - Diff between LocalFinalTask and FinalTask. Similarly
          // between InitiaTasl, InitialTaskWithLocalOutput
        ...
        // umbilical:  MRAppMaster creates (taskAttemptListener), passes to us
        umbilical.fsError(IDConverter.fromMRTaskAttemptId(classicAttemptID),
            e.getMessage());
        throw new RuntimeException();
        ...
        umbilical.reportDiagnosticInfo(
            IDConverter.fromMRTaskAttemptId(classicAttemptID), baos.toString());
        throw new RuntimeException();
        ...
        umbilical.fatalError(IDConverter.fromMRTaskAttemptId(classicAttemptID),
            cause);
        throw new RuntimeException();
      }
    }
   ...
   */
  private void runSubtask(MRTaskContext remoteTaskContext,
                          final TaskType taskType,
                          TaskAttemptId attemptID,
                          final int numMapTasks, 
                          boolean renameOutputs)
  throws RuntimeException, IOException {
    ...
    subTaskRunner.runSubtask(remoteTaskContext, taskType, attemptID,
        numMapTasks, renameOutputs);
  }
  
  // TODO EVENTUALLY Move elsewhere.
  private boolean isMapTask(TezTaskAttemptID taId) {
    MRTaskType mrTaskType = MRTaskType.fromString(taId.getTaskType());
    return mrTaskType == MRTaskType.MAP;
  }
  
  // TODO EVENTUALLY Move elsewhere.
  private boolean isReduceTask(TezTaskAttemptID taId) {
    MRTaskType mrTaskType = MRTaskType.fromString(taId.getTaskType());
    return mrTaskType == MRTaskType.REDUCE;
  }
  
  private int getPartition(TezTaskAttemptID taId) {
    return taId.getTaskID().getId();
  }
}
