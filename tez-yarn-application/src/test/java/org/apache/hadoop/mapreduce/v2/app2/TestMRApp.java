...
//  @Test
  ...
  public void testUpdatedNodes() throws Exception {
    ...
    verifyNumJobAttemptCompletionEvents(job, 2);

    // send updated nodes info
    NodeHealthStatus nhs = Records.newRecord(NodeHealthStatus.class);
    nhs.setIsNodeHealthy(false);
    NodeReport nr = Records.newRecord(NodeReport.class);
    nr.setNodeId(node1);
    nr.setNodeState(NodeState.UNHEALTHY);
    nr.setNodeHealthStatus(nhs);
    
    app.getContext().getEventHandler().handle(new AMNodeEventStateChanged(nr));

    app.waitForState(task1Attempt, TaskAttemptState.KILLED);
    app.waitForState(task2Attempt, TaskAttemptState.KILLED);
    app.waitForState(task3Attempt, TaskAttemptState.KILLED);

    // Reducer ends up getting killed since the node it was allocated on was
    // marked bad.

    // 2 map success. 2 map failures. 1 reduce killed.
    verifyNumJobAttemptCompletionEvents(job, 5);
    ...
    //3 map success. 2 map failure. 1 reduce killed.
    verifyNumJobAttemptCompletionEvents(job, 6);
    ...
    // 2 attempts for map1. OBSOLETE and SUCCEEDED.
    verifyNumJobAttemptCompletionEvents(job, 2);
    ...
    // 2 attempts for map1. 1 for map2.
    verifyNumJobAttemptCompletionEvents(job, 3);

    app.waitForState(reduceTask, TaskState.RUNNING);
    task3Attempt = reduceTask.getAttempts().values().iterator()
        .next();
    app.getContext()
        .getEventHandler()
        .handle(
    ...
    // 2 attempts for map1. 1 for map2. 1 for reduce.
    verifyNumJobAttemptCompletionEvents(job, 4);
  ...
  private void verifyNumJobAttemptCompletionEvents(Job job, int expected) {
    int retries = 0;
    TaskAttemptCompletionEvent[] events =
        job.getTaskAttemptCompletionEvents(0, 100);
    while (retries < 50 && events.length != expected) {
      try {
        Thread.sleep(200l);
    ...
    Assert.assertEquals("Expecting " + expected
        + " completion events for success", expected,
        events.length);
  }
