...
#########################################
# Mocking TezChild#main() env variables #
#########################################
...
echo "--> Starting TezChild..."

: "${TEZ_CHILD_HEAP_OPTS:="-Xmx1024m"}"
...
read -r -a HEAP_OPTS_ARR <<< "${TEZ_CHILD_HEAP_OPTS}"
...
exec java "${HEAP_OPTS_ARR[@]}" "${JAVA_OPTS_ARR[@]}" "${JAVA_ADD_OPENS[@]}" \
    ...
    -cp "$CLASSPATH" \
    org.apache.tez.runtime.task.TezChild \
    "$@"
