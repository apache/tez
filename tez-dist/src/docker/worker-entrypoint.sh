...
#########################################
# Mocking worker main env variables #
#########################################
...
export USER LOCAL_DIRS LOG_DIRS
...
echo "--> Starting Tez Worker..."

: "${TEZ_WORKER_HEAP_OPTS:="-Xmx1024m"}"
...
read -r -a HEAP_OPTS_ARR <<< "${TEZ_WORKER_HEAP_OPTS}"
...
exec java "${HEAP_OPTS_ARR[@]}" "${JAVA_OPTS_ARR[@]}" "${JAVA_ADD_OPENS[@]}" \
    ...
    -cp "$CLASSPATH" \
    org.apache.tez.runtime.task.StandaloneTezWorker \
    "$@"
