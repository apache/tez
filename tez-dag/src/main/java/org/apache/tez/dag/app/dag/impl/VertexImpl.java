...
public class VertexImpl implements org.apache.tez.dag.app.dag.Vertex, EventHandler<VertexEvent> {
  ...
  public VertexImpl(TezVertexID vertexId, VertexPlan vertexPlan,
    ...
    if (!isLocal) {
      taskSchedulerName =
          resolveFallbackName(taskSchedulerName, "Task scheduler", "scheduler", appContext::getTaskScheduerIdentifier,
              () -> appContext.getTaskSchedulerName(0));

      containerLauncherName = resolveFallbackName(containerLauncherName, "Container launcher", "launcher",
          appContext::getContainerLauncherIdentifier, () -> appContext.getContainerLauncherName(0));

      taskCommName = resolveFallbackName(taskCommName, "Task communicator", "communicator",
          appContext::getTaskCommunicatorIdentifier, () -> appContext.getTaskCommunicatorName(0));
    }
...
//  }
   ...
   */
  private String resolveFallbackName(String currentName, String logPrefix, String logSuffix,
                                     Function<String, Integer> identifierProvider, Supplier<String> fallbackProvider) {

    if (identifierProvider.apply(currentName) == null) {
      String fallback = fallbackProvider.get();
      if (fallback != null) {
        LOG.info("{} {} not found, using default {} {} at index 0", logPrefix, currentName, logSuffix, fallback);
        return fallback;
      }
    }
    return currentName;
