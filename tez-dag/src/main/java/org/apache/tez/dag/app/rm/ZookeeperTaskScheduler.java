...
public class ZookeeperTaskScheduler extends TaskScheduler {
        ...
        CuratorCacheListener.builder()
            .forCreates(
                node -> {
                  ...
                  String jsonStr = new String(node.getData(), StandardCharsets.UTF_8);
                  String host;
                  int shufflePort = 0;
                  try {
                    ...
                    shufflePort = jsonNode.get("shufflePort").asInt();
                  } catch (Exception e) {
                    LOG.warn("Failed to parse Jackson JSON payload from ZK, falling back to raw string. Payload: {}",
                        jsonStr);
                    host = jsonStr;
                  }

                  LOG.info("Discovered new TezChild via ZK: {} on host {} with shufflePort {}", containerIdStr, host,
                      shufflePort);
  ...
  private synchronized void tryAllocate() {
    TaskRequest request;
    Container container;
    // peek() is O(1) and safely checks for available items
    while (pendingTasks.peek() != null && availableContainers.peek() != null) {
      // Only poll if both are available
      request = pendingTasks.poll();
      container = availableContainers.poll();
      ...
      if (request != null && container != null) {
        ...
        // Safety net: shouldn't happen, but restore if it does
        if (request != null) {
